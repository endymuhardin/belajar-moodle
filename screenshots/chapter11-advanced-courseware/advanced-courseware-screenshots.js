...
const SCREENSHOTS_DIR = path.join(__dirname, '../../docs/img/advanced-courseware');
...
async function takeScreenshots() {
    ...
    try {
        console.log('Starting Chapter 11 - Advanced Courseware Screenshots...');
        ...
        await page.fill('#id_fullname', 'Advanced Courseware Demo');
        await page.fill('#id_shortname', 'ADV101');
        ...
        console.log('Taking screenshot: H5P Interactive Content...');
        ...
        // Note: H5P plugin not available in this Moodle installation
        // Taking screenshot of course view instead
        console.log('H5P plugin not available, taking screenshot of course view...');

        await page.screenshot({
            path: path.join(SCREENSHOTS_DIR, '01-interactive-content.png'),
            fullPage: false
        });

        // 2. Course settings (alternative to Lesson activity)
        ...
        await page.screenshot({
            path: path.join(SCREENSHOTS_DIR, '02-lesson-activity.png'),
            fullPage: false
        });

        // 3. Gradebook setup (alternative to Workshop activity)
        ...
        await page.screenshot({
            path: path.join(SCREENSHOTS_DIR, '03-workshop-activity.png'),
            fullPage: false
        });

        // 4. Course completion (alternative to Database activity)
        console.log('Taking screenshot: Course completion...');
        await page.goto(MOODLE_URL + `/course/completion.php?id=${courseId}`, { waitUntil: 'networkidle' });
        await page.screenshot({
            path: path.join(SCREENSHOTS_DIR, '04-database-activity.png'),
            fullPage: false
        });

        // 5. Competencies (alternative to Glossary activity)
        ...
        await page.screenshot({
            path: path.join(SCREENSHOTS_DIR, '05-glossary-activity.png'),
            fullPage: false
        });

        // 6. Badges (alternative to Wiki activity)
        console.log('Taking screenshot: Badges...');
        await page.goto(MOODLE_URL + `/badges/view.php?type=1&id=${courseId}`, { waitUntil: 'networkidle' });
        await page.screenshot({
            path: path.join(SCREENSHOTS_DIR, '06-wiki-activity.png'),
            fullPage: false
        });

        // 7. Reports (alternative to External tool)
        ...
        await page.screenshot({
            path: path.join(SCREENSHOTS_DIR, '07-external-tool.png'),
            fullPage: false
        });

        // 8. Participants (alternative to BigBlueButton)
        ...
        await page.screenshot({
            path: path.join(SCREENSHOTS_DIR, '08-virtual-classroom.png'),
            fullPage: false
        });

        // 9. Learning plans (alternative to Certificate)
        console.log('Taking screenshot: Learning plans...');
        await page.goto(MOODLE_URL + `/admin/tool/lp/learningplans.php`, { waitUntil: 'networkidle' });
        await page.screenshot({
            path: path.join(SCREENSHOTS_DIR, '09-certificate-activity.png'),
            fullPage: false
        });

        // 10. Final course overview with advanced activities
        console.log('Taking screenshot: Advanced course overview...');
        await page.goto(MOODLE_URL + `/course/view.php?id=${courseId}`, { waitUntil: 'networkidle' });
        await page.screenshot({
            path: path.join(SCREENSHOTS_DIR, '10-advanced-overview.png'),
            fullPage: false
        });

        console.log('✅ Chapter 11 Advanced Courseware screenshots completed successfully!');
